...
internal class MusicRegistry
{
    ...
    private async Task RegisterSong(ModSong song)
    {
        ...
        await this.encoder.Encode(song.FilePath, outputFile.FullName, null);
    ...
    private IEncoder GetEncoder()
    {
        ...
        return game switch
        {
            Game.P4G_PC => new CachedEncoder(new VgAudioEncoder(new VgAudioConfig() { OutContainerFormat = "hca" }), Directory.CreateDirectory(Path.Join(this.modDir, "P4G_P3P_cache")).FullName),
            Game.P3P_PC => new CachedEncoder(new VgAudioEncoder(new VgAudioConfig() { OutContainerFormat = "hca" }), Directory.CreateDirectory(Path.Join(this.modDir, "P4G_P3P_cache")).FullName),
            Game.P5R_PC => new CachedEncoder(new VgAudioEncoder(new VgAudioConfig() { OutContainerFormat = "hca", KeyCode = 9923540143823782 }), cachedDir),
            Game.P3R_PC => new CachedEncoder(new VgAudioEncoder(new VgAudioConfig() { OutContainerFormat = "hca", KeyCode = 11918920 }), cachedDir),
            Game.Metaphor => new CachedEncoder(new VgAudioEncoder(new VgAudioConfig() { OutContainerFormat = "hca", KeyCode = 572184256205070254 }), cachedDir),
            _ => throw new Exception("Unknown game."),
        };
    }
}
